...
<div class="box">
    ...
    <div class="box-body table-responsive">
        <table id="menu-tree" class="table table-hover">
            ...
            <tbody>
            <?php if(count($list)): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <tr <?php if($vo['week']=='周日'): ?>style="color:#fff;background-color: #e86261"<?php endif; ?>>
            <td><?php echo htmlentities($vo['date']); ?></td>
            <td><?php echo htmlentities($vo['week']); ?></td>
            <?php if(is_array($vo['date_schedules']) || $vo['date_schedules'] instanceof \think\Collection || $vo['date_schedules'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['date_schedules'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
            <td data-id="<?php echo htmlentities($item['id']); ?>" data-state="<?php echo htmlentities($item['state']); ?>" align="center" <?php if($item['state']==1): ?>class="action-edit bg-green" title="已开启"<?php elseif($item['state']==2): ?>class="action-edit bg-yellow" title="已预约"<?php elseif($item['state']==3): ?>class="action-edit bg-gray" title="已满房"<?php else: ?>class="action-edit"<?php endif; ?>>
                <?php echo substr($item['start_time'],11,5); ?> - <?php echo substr($item['end_time'],11,5); ?>
            </td>
            <td></td>
    ...
    $(function () {
        ...
        $("select[name=store_id]").change(function () {
            ...
            if(store_id){
                $.ajax({
                    type: 'post',
                    url: "<?php echo url('ajaxCounselorList'); ?>",

<?php
function send_wx_msg($key,$template_msg){
    $access_token = \think\Db::name('sys_cache')->where('sys_key',$key)->value('sys_value');
    $result = curl_post("https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=".$access_token,json_encode($template_msg));
...
function send_sms($mobile,$content){
    require_once '../vendor/ChuanglanSmsHelper/ChuanglanSmsApi.php';

    $clapi = new \ChuanglanSmsApi();

    //设置您要发送的内容：其中“【】”中括号为运营商签名符号，多签名内容前置添加提交
    $result = $clapi->sendSMS($mobile,'【芒果心理】'.$content );
